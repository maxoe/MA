...
\begin{align}\label{eq:multiple_pauses_pot}
...
% \begin{proof}
% 	Given a Graph $G=(V,E)$ with a set of parking nodes $P \subseteq V$, let $p = \langle s=v_0,v_1,\ldots,t=v_k, \rangle$ be a path in G with labels $l_i$ at nodes $v_i$. Let $l_{i-1}$ and $l_{i}$ label at nodes $v_{i-1}$ and $v_i$. In the proof of lemma \ref{lemma:pot_labels_get_larger} we considered the two cases of parking at $v_i$ and not parking at $v_i$. In the first case, the last used parking node of the two labels will not be the same, in the second case it will be the same node. We cannot simply differentiate these two cases now because we not only decide if we use a parking node or not, but we also decide how long the pause will be. Let $R$ be the set of driving time constraints $\restr_j$. If a label uses $v_i$ as a parking node with a pause time of $\restr_{p,k}$, then it fulfills the pause requirements for a pause for all driving time constraints $\restr_j$ with $0 < j \le k$. Therefore, $d_1(l_i) = \ldots = d_k(l_i) = 0$ and for $k < j \le |R|$ is the last parking node of $l_{i-1}$ and $l_i$ the same node which we name $p_j \in P \cup \{s\}$.

% 	It is

% 	\begin{align}
% 		\begin{split}\label{eq:label_n_feasibility_proof_1}
% 			d_0(l_{i}) + \pi_t(l_{i},v_{i}) & = \traveltime(s,v_{i}) + \drivingtime(v_i,t) + \sum_{j=1}^{n-1}{ n_j * \restr_{p,j}}\\
% 			& = \traveltime(s,v_{i}) + \drivingtime(v_i,t) + \sum_{j=1}^n{ \floor*{ \frac{d_{j}(l) + \drivingtime(v_i,t)}{\restr_{d,j}} } - \floor*{ \frac{d_{j+1}(l) + \drivingtime(v_i,t)}{t_{d,j+1}} } * \restr_{p,j}}\\
% 			& \phantom{{}=1} + \floor*{ \frac{d_{n}(l) + \drivingtime(v_i,t)}{\restr_{d,n}} }\\
% 			& = \drivingtime(s,v_{i}) + \drivingtime(v_{i},t) + \textit{pause time on s-t path}
% 		\end{split}
% 	\end{align}
...
% 	We $d_0(l_{i-1}) + \pi_t(l_{i-1},v_{i-1}) \le d_0(l_i) + \pi_t(l_i,v_i)$.

% 	\begin{align}
% 		\begin{split}\label{eq:label_n_feasibility_proof_2}
% 			d_0(l_{i-1}) + \pi_t(l_{i-1},v_{i-1}) &= \traveltime(s,v_{i-1}) + \drivingtime(v_{i-1},t)\\
% 			& + \sum_{j=1}^{n-1}{\left(\floor*{\frac{\drivingtime(p_j,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,j}}} - \floor*{\frac{\drivingtime(p_j,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,j+1}}}\right) * \restr_{d,j}}\\
% 			& + \floor*{\frac{\drivingtime(p_n,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,n}}} * \restr_{d,n}\\
% 			&= \traveltime(s,v_{i-1}) + \drivingtime(v_{i-1},t)\\
% 			& + \sum_{j=1}^{k-1}{\left(\floor*{\frac{\drivingtime(p_j,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,j}}} - \floor*{\frac{\drivingtime(p_j,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,j+1}}}\right) * \restr_{d,j}}\\
% 			& + \floor*{\frac{\drivingtime(p_n,v_{i-1}) + \drivingtime(v_{i-1},t)}{\restr_{p,n}}} * \restr_{d,n}
% 		\end{split}
% 	\end{align}
...
\begin{algorithm}[hbtp]
	...
	\While{\Q is not empty}
	{
		...
		\ForAll{ $(u,v) \in E$ }
		{
			\If{$d_0 + \omega(u,v) < \restr_d$}
			{
				$D \leftarrow \{(d_0 + \omega(u,v), d_1 + \omega(u,v))\}$

				\If{$v \in P$}
				{
					$D$.\listInsert{$(d_0 + \omega(u,v) + \restr_p, 0)$}
				}
